...
// ==========================================================
// --- NOUVEAU (13 sept 2026) : bouton "Transfert" — changement de
// collecteur d'un membre via zone → collecteur. Réutilise
// reassignerMembresSpecifiques (module 1), qui transporte le solde
// automatiquement et laisse la commission déjà verrouillée chez l'ancien
// collecteur ("sauf la commission déjà déduite").
// ==========================================================
function ouvrirTransfertMembre(uid, nom) {
  const collecteursTousActifs = state.users.filter((u) => u.role === "collecteur" && u.statut !== "supprime");
  const prefectures = Array.from(new Set(
    collecteursTousActifs.map((c) => c.prefecture && c.prefecture.trim() ? c.prefecture.trim() : "Non précisé")
  )).sort((a, b) => a.localeCompare(b, "fr"));

  ouvrirModal(`
    <h2>Transférer ${nom}</h2>
    <p class="subtitle-sm">Choisissez la zone puis le nouveau collecteur. Le solde d'épargne du membre sera transporté automatiquement ; la commission déjà verrouillée reste chez l'ancien collecteur.</p>
    <div class="field-row">
      <label>Préfecture / Commune</label>
      <select id="transfert-select-prefecture">
        <option value="">-- Choisir --</option>
    ...
    <div class="field-row">
      <label>Sous-préfecture / Quartier</label>
      <select id="transfert-select-sous-prefecture" disabled>
        <option value="">-- Choisir la préfecture d'abord --</option>
      </select>
    </div>
    <div class="field-row">
      <label>Nouveau collecteur</label>
      <select id="transfert-select-collecteur" disabled>
        <option value="">-- Choisir la sous-préfecture d'abord --</option>
      </select>
    </div>
    <div class="modal-actions">
      <button type="button" class="btn btn-ghost-sm" id="modal-annuler" style="flex:1;">Annuler</button>
      <button type="button" class="btn btn-primary" id="btn-confirmer-transfert" style="flex:1;">Transférer</button>
    </div>
  `);

  document.getElementById("modal-annuler").addEventListener("click", fermerModal);

  document.getElementById("transfert-select-prefecture").addEventListener("change", (e) => {
    const pref = e.target.value;
    const selectSousPref = document.getElementById("transfert-select-sous-prefecture");
    const selectCollecteur = document.getElementById("transfert-select-collecteur");
    selectCollecteur.innerHTML = `<option value="">-- Choisir la sous-préfecture d'abord --</option>`;
    selectCollecteur.disabled = true;
    if (!pref) {
      selectSousPref.innerHTML = `<option value="">-- Choisir la préfecture d'abord --</option>`;
      selectSousPref.disabled = true;
      return;
    }
    const sousPrefs = Array.from(new Set(
      collecteursTousActifs
        .filter((c) => (c.prefecture && c.prefecture.trim() ? c.prefecture.trim() : "Non précisé") === pref)
        .map((c) => (c.sous_prefecture && c.sous_prefecture.trim() ? c.sous_prefecture.trim() : "Non précisé"))
    )).sort((a, b) => a.localeCompare(b, "fr"));
    selectSousPref.disabled = false;
    selectSousPref.innerHTML = `<option value="">-- Choisir --</option>` +
      sousPrefs.map((s) => `<option value="${s}">${s}</option>`).join("");
  });

  document.getElementById("transfert-select-sous-prefecture").addEventListener("change", (e) => {
    const pref = document.getElementById("transfert-select-prefecture").value;
    const sousPref = e.target.value;
    const selectCollecteur = document.getElementById("transfert-select-collecteur");
    if (!sousPref) {
      selectCollecteur.innerHTML = `<option value="">-- Choisir la sous-préfecture d'abord --</option>`;
      selectCollecteur.disabled = true;
    ...
    const collecteursZone = collecteursTousActifs.filter((c) =>
      (c.prefecture && c.prefecture.trim() ? c.prefecture.trim() : "Non précisé") === pref &&
      (c.sous_prefecture && c.sous_prefecture.trim() ? c.sous_prefecture.trim() : "Non précisé") === sousPref
    );
    selectCollecteur.disabled = false;
    selectCollecteur.innerHTML = `<option value="">-- Choisir --</option>` +
      collecteursZone.map((c) => `<option value="${c.uid}">${c.nom}</option>`).join("");
  });

  document.getElementById("btn-confirmer-transfert").addEventListener("click", async () => {
    const collecteurId = document.getElementById("transfert-select-collecteur").value;
    if (!collecteurId) {
      notifier("Choisissez un collecteur de destination.", "erreur");
      return;
    }
    try {
      await reassignerMembresSpecifiques([uid], collecteurId);
      notifier(`${nom} a été transféré(e) vers le nouveau collecteur.`, "succes");
      fermerModal();
    } catch (err) {
      console.error(err);
      notifier("Erreur : " + err.message, "erreur");
    ...
    return `
      <div class="entity-card" data-uid="${m.uid}">
          ...
          <div class="entity-actions">
            ...
            <button class="btn btn-secondary btn-sm" data-action="transferer-membre" data-uid="${m.uid}" data-nom="${m.nom}">Transfert</button>
...
document.getElementById("liste-membres").addEventListener("click", (e) => {
  const btnTransfert = e.target.closest("button[data-action='transferer-membre']");
  if (btnTransfert) {
    const { uid, nom } = btnTransfert.dataset;
    ouvrirTransfertMembre(uid, nom);
    return;
  }
